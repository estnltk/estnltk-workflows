#
#   This module contains a modified version of BertNerTagger.
#
#   Based on: 
#   https://github.com/estnltk/estnltk/blob/devel_1.7/estnltk_neural/estnltk_neural/taggers/ner/bert_ner_tagger.py
#   ( commit a0bafd204e15da4c12d26b0330b8822ccfe51515 ) 
#
...
def estbert_normalizer( input_str ):
    '''Fixes obscure string combinations on which the "estbertner_v1" model fails 
...
class BertNerTagger(MultiLayerTagger):
    ...
    conf_param = ('model_location', 'nlp', 'tokenizer', 'custom_words_layer', 'batch_size', 
                  ...
                  'aggregation_strategy', 'stride', 'use_fast', 'model_max_length', 'input_normalizer', 
    ...
    def __init__(self, model_location: str = None, output_layer: str = 'estbertner', 
                       ...
                       input_normalizer: Callable[[str], str] = estbert_normalizer 
                       ):
        """
        ...
        batch_size: int
            ...
            Note: this is `BertNerTagger`'s internal `batch_size` parameter, not to be confused 
            with `batch_size` parameter of the `transformers.pipeline`.
            Default: 1750
        stride: int 
            The `stride` argument to be passed to `transformers.pipeline`. 
            According to transformers' documentation: "If `stride` is provided, the pipeline is 
            applied on all the text. The text is split into chunks of size `model_max_length`. 
            Works only with fast tokenizers and `aggregation_strategy` different from `NONE`. 
            The value of this argument defines the number of overlapping tokens between chunks. 
            In other words, the model will shift forward by `tokenizer.model_max_length` - `stride` 
            tokens each step."
            Use this parameter along with `aggregation_strategy` and `use_fast` to switch on 
            built-in chucking capabilities of the `transformers.pipeline`. 
            Default: None
        aggregation_strategy: str
            The `aggregation_strategy` argument to be passed to `transformers.pipeline`. Optional. 
            According to transformers' documentation: "The strategy to fuse (or not) tokens based 
            on the model prediction." Possible values: "none", "simple", "first", "average", "max". 
            Use this parameter along with `stride` and `use_fast` to switch on built-in chucking 
            capabilities of the `transformers.pipeline`. 
            Default: None
        use_fast: bool
            The `use_fast` argument to be passed to `transformers.tokenizer`. Optional. 
            Use this parameter along with `stride` and `aggregation_strategy` to switch on built-in 
            chucking capabilities of the `transformers.pipeline`. 
            Default: False
        model_max_length: int
            The `model_max_length` argument to be passed to `transformers.tokenizer`. 
            Default: 512
        ...
        self.nlp = pipeline("ner", model=bertner, tokenizer=tokenizer, stride=stride, 
    ...
    def _make_layers(self, text: Text, layers: MutableMapping[str, Layer], status: dict) -> Layer:
        ...
        for text_chunk, (chunk_start, chunk_end) in zip(text_chunks, text_indexes):
            ...
            #print(f'Processing chunk {chunk_count} out of {len(text_chunks)} chunks:')
            #print(f'{text_chunk!r}')
            #print()
            ...
            if self.aggregation_strategy is None:
                ...
                #
                # Use a built-in aggregation_strategy
                #
                token_id = 0
                for detected_entity in response:
                    entity_type = detected_entity['entity_group']
                    entity_start = detected_entity['start']
                    entity_end = detected_entity['end']
                    # collect corresponding Bert tokens by start,end indexes
                    entity_spans = []
                    while token_id < len(bert_tokens):
                        token_start = bert_tokens[token_id][0]
                        token_end   = bert_tokens[token_id][1]
                        ...
                        if token_end <= entity_start:
                            token_id += 1
                            continue
                        elif token_start == entity_start:
                            corrected_start = chunk_start + token_start
                            corrected_end   = chunk_start + token_end
                            entity_spans.append( ElementaryBaseSpan(corrected_start, corrected_end) )
                        elif entity_start < token_start and token_end < entity_end:
                            corrected_start = chunk_start + token_start
                            corrected_end   = chunk_start + token_end
                            entity_spans.append( ElementaryBaseSpan(corrected_start, corrected_end) )
                        elif token_end <= entity_end:
                            corrected_start = chunk_start + token_start
                            corrected_end   = chunk_start + token_end
                            entity_spans.append( ElementaryBaseSpan(corrected_start, corrected_end) )
                        elif entity_end <= token_start:
                            break
                        token_id += 1
                    if entity_spans:
                        nerlayer.add_annotation(EnvelopingBaseSpan(entity_spans), 
                        ...
                        raise Exception(f'(!) Could not collect Bert tokens corresponding to entity {detected_entity!r}')
...
def _split_text_into_smaller_texts(large_text: Text, max_size:int=1750, \
    ...
    '''Splits given large_text into smaller texts following the text size limit. 
       ...
       on which EstBert (or some other model) is less likely to fail -- basically it involves 
       replacing some bad characters which can cause internal indexing errors. 
