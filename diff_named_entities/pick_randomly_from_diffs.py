...
if __name__ == '__main__':
    ...
    parser.add_argument('ann_diffs_file', type=str, \
                        help="file containing named entity annotation differences. "+\
                             "must be *__ann_diffs_* file created by one of the scripts "+\
                             "that finds named entity differences (e.g. diff_ner_tagger.py).")
    ...
    with open(diff_file, 'r', encoding='utf-8') as in_f:
        ...
        for line in in_f:
            ...
            if pattern_separator in line:
                ...
                collected_lines = []
                pickable_line = False
    assert len(lines_of_rand_picked_examples.keys()) == rand_pick
