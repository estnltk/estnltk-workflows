#
#  Carries over original 'words' layer from 
...
def make_new_words_layer( old_text_obj, new_text_obj, new_layer='original_words', old_layer='words' ):
    ...
    layer = Layer(name = new_layer,
                  ...
                  parent   =None,
                  ambiguous=True)
    layer.meta['desc'] = 'Original word tokenization from koondkorpus XML file.'
    for span in original_layer:
        attribs = {'normalized_form':None}
        layer.add_annotation( (span.start, span.end), **attribs )
    return layer

def carry_over_words_row_mapper( row ):
    ...
    src_texts = fetch_src_documents( src_keys, src_collection, target_text=new_text, layers=['words'] )
    assert len(src_texts) > 0, '(!) No matching text found for doc hash {!r}'.format(target_hash)
    src_key, src_text_obj = src_texts[0]
    layer = make_new_words_layer( src_text_obj, new_text, new_layer='original_words', old_layer='words' )
...
try:
    ...
    trg_collection.create_layer(layer_name='original_words', 
                                data_iterator=data_iterator, 
                                row_mapper=carry_over_words_row_mapper, 
                                tagger=None,
                                progressbar='ascii',
                                mode='overwrite')
    '''
    ...
    for tkey, target_text  in  data_iterator:
        ...
        assert target_hash in src_hash_index, '(!) Doc hash {!r} cannot be mapped to {!r}'.format(target_hash, SOURCE_COLLECTION)
        src_keys = src_hash_index[target_hash]
        src_texts = fetch_src_documents( src_keys, src_collection, target_text=target_text, layers=['words'] )
        assert len(src_texts) > 0, '(!) No matching text found for doc hash {!r}'.format(target_hash)
        new_layer = make_new_words_layer( src_texts[0][1], target_text, new_layer='original_words', old_layer='words' )
    '''
finally:
    src_storage.close()
    trg_storage.close()